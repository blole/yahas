...
public class TimeoutHashSet<K> {
	...
	private void forkReportingDataNodeDisconnections(final long timeout_ms, final Action<K> onRemoval) {
		new Thread() {
			@Override
			public void run() {
				...
				while (true) {
					try {
						long timeTillNextTimeout = soonestTimeout - System.currentTimeMillis();
					...
					long oldestAllowedTime = System.currentTimeMillis() - timeout_ms;
					for (Entry<K, Long> e : hashMap.entrySet()) {
						if (e.getValue() < oldestAllowedTime)
							toBeRemoved.add(e.getKey());
						else
							soonestTimeout = Math.min(soonestTimeout, e.getValue());
					}
